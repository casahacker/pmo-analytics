      ...
      <div className="flex justify-between items-center bg-card p-6 rounded border border-line">
        <div className="flex gap-4 items-end">
          ...
          <div className="flex gap-2 mb-0.5 ml-4">
            <select value={reportMonth} onChange={(e) => setReportMonth(parseInt(e.target.value))} className="bg-sidebar border border-line text-xs font-bold uppercase text-text rounded px-2 py-1 outline-none focus:border-primary transition-colors">
              {MONTHS_PT.map((m, i) => (
                <option key={m} value={i}>{m}</option>
              ))}
            </select>
            <select value={reportYear} onChange={(e) => setReportYear(parseInt(e.target.value))} className="bg-sidebar border border-line text-xs font-bold uppercase text-text rounded px-2 py-1 outline-none focus:border-primary transition-colors">
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 mr-4">
            <span className="text-xs text-text-secondary font-bold uppercase">Parecer PMO:</span>
      ...
      <div className="bg-white text-text rounded p-8 print:p-0 print:shadow-none min-h-[800px] text-left flex flex-col border border-line">
        <header className="flex justify-between border-b-2 border-text pb-6 mb-8 items-end">
          ...
          <div className="flex flex-col items-end">
            ...
            <span className="text-xs font-bold text-text-secondary">Ciclo Monitorado: {MONTHS_PT[reportMonth]} / {reportYear}</span>
          </div>
        </header>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
          <div className="space-y-4">
            <div className="bg-success/5 p-4 border border-success/20 rounded">
              <h5 className="text-xs font-bold uppercase text-success mb-4 border-b border-success/20 pb-2 flex justify-between">
              ...
              <div className="space-y-2">
                {reportIssues.filter(i => i.statusCategory === "done").map(issue => (
                  <div key={issue.key} className="flex gap-3 items-start border-b border-success/10 pb-2">
                    <span className="text-xs font-mono font-bold text-success shrink-0">{issue.key}</span>
          ...
          <div className="space-y-4">
            <div className="bg-error/5 p-4 border border-error/20 rounded h-full flex flex-col">
              <h5 className="text-xs font-bold uppercase text-error mb-4 border-b border-error/20 pb-2 flex justify-between">
              ...
              <div className="space-y-2 flex-grow">
                {reportIssues.filter(i => i.statusCategory !== "done").map(issue => (
                  <div key={issue.key} className="flex gap-3 items-start border-b border-error/10 pb-2">
                    <span className="text-xs font-mono font-bold text-error shrink-0">{issue.key}</span>
                    <div className="space-y-0.5">
                      <p className="text-xs font-medium leading-tight text-text">{issue.summary}</p>
                      <p className="text-xs font-bold uppercase text-error text-left">
              ...
              <div className="mt-4 pt-3 border-t border-error/20">
                <p className="text-xs font-bold text-error uppercase tracking-wide mb-1.5 opacity-60">Significado dos Status de Gargalo:</p>
                <div className="grid grid-cols-1 gap-y-1.5">
                  {[["Atraso", "Task com prazo original do Jira vencido"], ["Impedimento", "Task bloqueada por dependência externa"], ["Inativo", "Sem movimentação no Ticket por mais de 15 dias"], ["Aguardando Alocação", "Task no fluxo do mês aguardando execução"]].map(([label, desc]) => (
                    <div key={label} className="flex items-center gap-1.5 grayscale">
                      ...
                      <span className="text-xs text-error font-bold leading-none uppercase"><span className="text-error">{label}:</span> {desc}</span>
                    </div>
                  ))}
                </div>
              </div>
        ...
        <div className="bg-bg p-6 rounded border border-line mb-12">
          ...
          <div className="grid grid-cols-3 gap-6">
            <div className="space-y-1 text-left">
              ...
              <p className="text-xs text-text-secondary leading-tight">Métrica baseada na completude das informações de tarefas e subtarefas.</p>
            </div>
            <div className="space-y-1 text-left">
              ...
              <p className="text-xs text-text-secondary leading-tight">Volume de entregas com cronograma comprometido ou vencido.</p>
            </div>
            <div className="space-y-1 text-left">
              ...
              <p className="text-xs text-text-secondary leading-tight">Total de demandas monitoradas neste ciclo de status.</p>
            </div>
          </div>
        </div>
        <div className="mt-12 grid grid-cols-2 gap-12">
          ...
          <div className="flex flex-col justify-end items-center pb-2">
            <div className="w-full border-t border-text pt-2 text-center">
              <p className="text-xs font-bold uppercase text-text">Assinatura Responsável Técnico</p>
              <p className="text-xs font-bold text-text-secondary uppercase tracking-wide">ASSOCIAÇÃO CASA HACKER</p>
            </div>
          </div>
        </div>
        <div className="mt-auto pt-16 border-t border-line flex flex-col gap-6 text-text-secondary">
          <div className="flex justify-between items-start">
            <div className="space-y-1 text-left">
              ...
              <p className="text-xs font-bold uppercase text-text-secondary">{selectedProject === "All" ? "Relatório Global" : (projectData.find(p => p.key === selectedProject)?.name)} - {selectedProject}</p>
